 ...
 * @see it.castelli.Memory.game.medium.MediumDifGamePanel
 * @author Maltempi Elisabetta 4CI
 * @author Togni Claudio 4CI
 * @version 1.7
 */
@SuppressWarnings("serial")
public class MediumDifGameFrame extends JFrame{
	...
	private Card card1;
	private Card card2;
	private MediumDifGamePanel gamePanel;
	private Alert alert = new Alert();
	private int tentativi = 0;
	private boolean loggedIn = false;
	private String username = "";
	
    public MediumDifGameFrame(ImageCategory category){
    	this.category = category;
    	this.gamePanel = new MediumDifGamePanel(category);
    	setConfirmButtonProperties();
    	setFrameProperties();
    }
    
    public MediumDifGameFrame(boolean loggedIn, String username, ImageCategory category){
    	this.category = category;
    	...
    	confBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(completed) { 
					...
					if(loggedIn) {
						new PointSaving(username, tentativi, "Medium");
					}else {
						new PointSaving("guest", tentativi, "Medium");
					}
